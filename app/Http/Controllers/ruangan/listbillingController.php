...
class listbillingController extends Controller
{
    public function index(Request $request)
    {
        return view('ruangan.list_billing');
    }
    public function datasetlistbilling(Request $request)
    {
        ...
        //cek data billing terclose
        $cekdatanori = DB::connection('mysql')->table('tr_billing')
            ->select('nori')
            ->where('kdlokasi', $kdlokasi)
            ->where('kode1', $kode1)
            ->where('tglkeluar', 0000 - 00 - 00)
            ->get();
        foreach ($cekdatanori as $cekdatanori2) {
            //ambil tanggal keluar pasien
            $tglkeluarpasien = $connection->table('dafinap')
                ->select(
                    [
                        'nori', 'tglkeluar'
                    ]
                )
                ->where('nori', '=', $cekdatanori2->nori)
                ->get();
            // update tgl keluar pasien dari simrs
            $updatedatabilling = DB::connection('mysql')->table('tr_billing')
                ->where('nori', '=', $tglkeluarpasien['0']->nori)
                ->where('kdlokasi', '=', Auth::user()->kdlokasi)
                ->where('kode1', '=', Auth::user()->kode1)
                ->update([
                    'tglkeluar' => $tglkeluarpasien['0']->tglkeluar
                ]);
        }
        //ambil data pasien ranap di simrs
        $pasranap = $connection->table('dafinap')
            ->select(
                [
                    ...
                    'dafinap.kdkons', 'dafinap.nmkons', 'dafinap.noasuransi', 'nosep', 'jthkelas', 'tgllahir', 'tglmasuk', 'tglkeluar'
                ]
            )
                            ...
                            foreach ($totalreturobat as $billreturobat) {
                                ...
                                foreach ($adm as $admin) {
                                    ...
                                    foreach ($billingtindakanadm as $billadm) {
                                        ...
                                        //perhitungan golongan umur pasien
                                        $tglskg = Carbon::now();
                                        ...
                                        //insert data tarikan ke table transaksi enigma
                                        ...
                                        if ($cekdatainsert == null) {
                                            $insertdatabilling = DB::connection('mysql')->table('tr_billing')->insert(
                                                [
                                                    ...
                                                    'tglmasuk' => $datapasien->tglmasuk,
                                                    'tglkeluar' => $datapasien->tglkeluar,
                                            ...
                                            //update transaksi berdasar tarikan awal dari simrs
    ...
    public function parsingdatapasbil(request $request)
    {
        ...
        if ($parsedata->isnotEmpty()) {
            ...
            $kunjung = DB::connection('mysql')->table('mst_kriteria_kunj')
                ...
                ->first();
            $kelasnaik = DB::Connection('mysql')->table('tr_billing')
                ->select([
                    'stts_naikkelas',
                    'naikkelas',
                    'ms_kls_rwt.kls_rwt'
                ])
                ->join('ms_kls_rwt', 'tr_billing.naikkelas', '=', 'ms_kls_rwt.id')
                ->where('nori', '=', $nori)
                ->first();
            if ($kelasnaik == null) {
                $kls_naik = 0;
                ...
                $kls_naik = $kelasnaik->kls_rwt;
                $stts_naik = $kelasnaik->stts_naikkelas;
                $idkelas = $kelasnaik->naikkelas;
            }
            $parsedatapasbil[] = [
    ...
    public function updateina(request $request)
    {
        ...
        if ($updateina == 0 || $updateina == 1) {
            if ($kelasbaru == null || $kelasbaru == ' ') {
                $sttsnaik = 0;
            } else {
                $sttsnaik = 1;
            }
    ...
    public function naikkelas(Request $request)
    {
        ...
        $kode1 = Auth::user()->kode1;
        $kelasbaru = $request->kelasbaru;
        $naikkelas = DB::connection('mysql')->table('tr_billing')->where('nori', '=', $nori)->where('kdlokasi', '=', Auth::user()->kdlokasi)->where('kode1', '=', $kode1)->update(
            [
                'kdlokasi' => Auth::user()->kdlokasi,
                'kode1' => $kode1,
                'icd10_1' => $request->icd10_1,
        ...
        if ($naikkelas) {
            return response($naikkelas, 200);
        } else {
            return response("Data Gagal Diupdate");
        }
    }
    public function batalnaikkelas(Request $request)
    {
        $nori = $request->nori;
        $kode1 = Auth::user()->kode1;
        $batalnaikkelas = DB::connection('mysql')->table('tr_billing')->where('nori', '=', $nori)->where('kdlokasi', '=', Auth::user()->kdlokasi)->where('kode1', '=', $kode1)->update(
            [
                'kdlokasi' => Auth::user()->kdlokasi,
                'kode1' => $kode1,
                'icd10_1' => $request->icd10_1,
        ...
        if ($batalnaikkelas) {
            return response($batalnaikkelas, 200);
        } else {
            return response("Data Gagal Diupdate");
        }
    }
