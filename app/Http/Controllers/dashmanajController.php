...
class dashmanajController extends Controller
{
    public function index(Request $request)
    {

        return view('dashboardmanaj');
    }
    ...
    public function pilihunit(Request $request)
    {
        ...
        if ($input == '') {
            ...
            $dataunit = $connection->table('tindakan1')->where('nama', 'like', '%' . $input . '%')->get()->toArray();
            if (empty($dataunit)) {
                $dataunit = $connection->table('tindakan1')->where('kode1', 'like', '%' . $input . '%')->get()->toArray();
            }
        }
        return response()->json($dataunit);
    }
    public function pilihchart(Request $request)
    {
        $input = $request->search;
       
        if ($input == '') {
            $pilihchart = DB::connection('mysql')->table('ms_kategori_chart')
        ...
        return response()->json($pilihchart);
    }
